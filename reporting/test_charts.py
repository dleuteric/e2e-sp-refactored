...
def main():
    ...
    try:
        ...
        if not data.has_mgm:
            print("[FAIL] ERROR: No MGM data available")
            return 1

        print(f"[OK] MGM loaded: {len(data.mgm)} samples")
    ...
    try:
        ...
        if fig is None:
            print("[FAIL] ERROR: Figure generation returned None")
            return 1

        print(f"[OK] Figure generated: {fig.layout.title.text}")

    except Exception as e:
        print(f"[FAIL] ERROR generating figure: {e}")
        import traceback
        traceback.print_exc()
        return 1
    ...
    try:
        ...
        print(f"[OK] PNG exported: {exported_path.name}")
        print(f"  Size: {size_kb:.1f} KB")

    except Exception as e:
        print(f"[FAIL] ERROR exporting PNG: {e}")
        import traceback
        traceback.print_exc()
        return 1
    ...
    if png_size < 10:
        ...
        print(f"[OK] PNG validated: {png_size:.1f} KB")
    ...
    if pdf_size < 20:
        ...
        print(f"[OK] PDF validated: {pdf_size:.1f} KB")
