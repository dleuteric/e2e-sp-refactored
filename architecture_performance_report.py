...
#     """
#     Build 2D network topology on a real map (dark theme) using Plotly geo.
#     Coastlines/countries/land are enabled via `update_geos`.
#     Links are STRAIGHT in equirectangular projection.
#     """
#
#     fig = go.Figure()
#
...
#         return lat, lon
#
#     # Helper: Add STRAIGHT line segment in equirectangular projection
#     def add_seg(lat1, lon1, lat2, lon2, color, width, dash, name, showlegend=False):
#         # Skip if crosses dateline
#         if abs(lon2 - lon1) > 180:
#             return
#
#         # Interpolate intermediate points for straight line in lat/lon space
#         n_points = 50  # More points = straighter line
...
#             active_dnl.add(edge)
#
#     # Idle ISL = all sat pairs minus active (sample for clutter)
#     all_pairs = set()
#     for i, sa in enumerate(all_sats):
#         for sb in all_sats[i + 1:]:
#             all_pairs.add(tuple(sorted([sa, sb])))
#     idle_isl = list(all_pairs - active_isl)
#     idle_isl = idle_isl[:min(20, len(idle_isl))]
#
#     # Idle downlinks: geometry (elevation) gate only
...
#             la, loa = sat_latlon[a]
#             lb, lob = sat_latlon[b]
#             show = not legend['ISL idle']
#             add_seg(la, loa, lb, lob, 'rgba(0,255,255,0.15)', 0.5, 'dot', 'ISL idle', show)
...
#         else:
#             continue
#         show = not legend['Downlink idle']
#         add_seg(ls, los, lg, log, 'rgba(77,255,77,0.2)', 0.5, 'dot', 'Downlink idle', show)
...
#             la, loa = sat_latlon[a]
#             lb, lob = sat_latlon[b]
#             show = not legend['ISL active']
#             add_seg(la, loa, lb, lob, 'rgba(0,255,255,0.7)', 1.5, 'solid', 'ISL active', show)
...
#         else:
#             continue
#         show = not legend['Downlink active']
...
#             marker=dict(size=10, color='#e74c3c', symbol='diamond', line=dict(width=1.5, color='white')),
#             name='Target',
...
#     # Satellite markers
#     if sat_latlon:
#         lats = [v[0] for v in sat_latlon.values()]
#         lons = [v[1] for v in sat_latlon.values()]
#         names = list(sat_latlon.keys())
#         fig.add_trace(go.Scattergeo(
#             lat=lats, lon=lons, mode='markers', text=names,
#             marker=dict(size=5, color='#3498db', symbol='circle', line=dict(width=0.5, color='white')),
#             name='Satellites',
...
#             marker=dict(size=8, color='#2ecc71', symbol='square', line=dict(width=1, color='white')),
#             name='Ground Stations',
#             hovertemplate='<b>%{text}</b><br>Lat: %{lat:.2f}°<br>Lon: %{lon:.2f}°<extra></extra>'
#         ))
#
#     # Map layout - DARK THEME with coastlines
#     fig.update_geos(
