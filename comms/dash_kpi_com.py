...
measurement age tracking, and professional visual effects.
...
SAMPLE_EVERY: int = 50  # take 1 of N LOS events
MAX_FRAMES: Optional[int] = None
FPS: int = 30

# Measurement age CSV paths
ONBOARD_CSV: Optional[Path] = Path(
    "/Users/daniele_leuteri/PycharmProjects/tracking-perf/tracking-perf-comms/exports/gpm/20251012T111734Z/triangulation_onboard_20251012T111734Z__HGV_954.csv"
)
GROUND_CSV: Optional[Path] = Path(
    "/Users/daniele_leuteri/PycharmProjects/tracking-perf/tracking-perf-comms/exports/gpm/20251012T111734Z/triangulation_only_20251012T111734Z__HGV_954.csv"
)
...
COLOR_EARTH_CONTINENTS = 'rgb(120, 180, 120)'  # Land green

# Satellite states (distinctive colors)
COLOR_SAT_IDLE = dict(color='rgba(150, 150, 170, 0.6)', size=3, symbol='circle')
COLOR_SAT_TRACKING = dict(color='rgba(255, 120, 0, 1)', size=8, symbol='diamond')
COLOR_SAT_TRACKING_ISL = dict(color='rgba(0, 255, 255, 1)', size=8, symbol='cross')
COLOR_SAT_TRACKING_DNL = dict(color='rgba(120, 255, 0, 1)', size=8, symbol='square')
COLOR_SAT_TRANSMITTING = dict(color='rgba(0, 255, 120, 1)', size=10, symbol='square')
COLOR_SAT_ISL_ACTIVE = dict(color='rgba(0, 200, 255, 1)', size=6, symbol='cross')
...
# Links (distinctive)
STYLE_ISL = dict(color='rgba(0, 180, 255, 0.7)', width=1, dash='solid')
STYLE_ISL_ACTIVE = dict(color='rgba(0, 255, 255, 1)', width=3, dash='solid')
STYLE_DOWNLINK = dict(color='rgba(0, 255, 120, 0.8)', width=2, dash='dash')
STYLE_DOWNLINK_ACTIVE = dict(color='rgba(120, 255, 0, 1)', width=4, dash='solid')
STYLE_LOS = dict(color='rgba(255, 100, 200, 0.6)', width=1, dash='dot')
STYLE_ROUTE = dict(color='rgba(255, 220, 0, 1)', width=5, dash='solid')

# Overlay/perf knobs
DRAW_ALL_ACTIVE_ISL: bool = True
DRAW_ALL_ACTIVE_DNL: bool = True
MAX_LOS_LINES: int = 50
TARGET_TRAIL_LENGTH: int = 20
...
def load_measurement_ages() -> Tuple[Optional[pd.DataFrame], Optional[pd.DataFrame]]:
    """Load measurement age data from CSV files"""
    onboard_df = None
    ground_df = None

    if ONBOARD_CSV and ONBOARD_CSV.exists():
        try:
    ...
    if GROUND_CSV and GROUND_CSV.exists():
        try:
    ...
    return onboard_df, ground_df


def get_measurement_age_at_time(t: float, onboard_df: Optional[pd.DataFrame],
                                ground_df: Optional[pd.DataFrame]) -> Tuple[Optional[float], Optional[float]]:
    """Get interpolated measurement age at time t"""
    onboard_age = None
    ground_age = None

    if onboard_df is not None and len(onboard_df) > 0:
        # Find nearest time
        idx = np.searchsorted(onboard_df['epoch_s'].values, t)
        if idx > 0 and idx < len(onboard_df):
            # Linear interpolation
            t0, t1 = onboard_df['epoch_s'].iloc[idx - 1], onboard_df['epoch_s'].iloc[idx]
            a0, a1 = onboard_df['age_total_ms'].iloc[idx - 1], onboard_df['age_total_ms'].iloc[idx]
            alpha = (t - t0) / (t1 - t0) if t1 != t0 else 0
            onboard_age = a0 + alpha * (a1 - a0)
        elif idx == 0:
            onboard_age = onboard_df['age_total_ms'].iloc[0]
        else:
            onboard_age = onboard_df['age_total_ms'].iloc[-1]

    if ground_df is not None and len(ground_df) > 0:
        # Find nearest time
        idx = np.searchsorted(ground_df['epoch_s'].values, t)
        if idx > 0 and idx < len(ground_df):
            # Linear interpolation
            t0, t1 = ground_df['epoch_s'].iloc[idx - 1], ground_df['epoch_s'].iloc[idx]
            a0, a1 = ground_df['age_total_ms'].iloc[idx - 1], ground_df['age_total_ms'].iloc[idx]
            alpha = (t - t0) / (t1 - t0) if t1 != t0 else 0
            ground_age = a0 + alpha * (a1 - a0)
        elif idx == 0:
            ground_age = ground_df['age_total_ms'].iloc[0]
        else:
            ground_age = ground_df['age_total_ms'].iloc[-1]

    return onboard_age, ground_age
...
# ----------------------------------------------------------------------------
# 🚀 ENHANCED PLOT DYNAMIC WITH MEASUREMENT AGE
# ----------------------------------------------------------------------------
...
def plot_dynamic(run_id: str, cfg: dict) -> None:
    ...
    # Load measurement age data
    onboard_df, ground_df = load_measurement_ages()
    ...
    if SAVE_HTML and _PLOTLY_OK:
        print("🎨 Building enhanced HTML visualization with measurement age tracking...")
        ...
        def classify_satellites_enhanced(t: float, route: Optional[List[str]], src_id: str) -> Dict[str, List[str]]:
            """Enhanced classification with combined states"""
            states = {
                'idle': [],
                'tracking': [],
                'tracking_isl': [],  # Tracking + ISL active
                'tracking_dnl': [],  # Tracking + Downlink active
                'transmitting': [],
                'isl_active': []
            }
            ...
            for a, b, data in net.graph.edges(data=True):
                ...
                elif "gnd" in str(a) or "gnd" in str(b):
                    if "satellite" in net._nodes.get(a, type(None).__new__(type)).node_type if a in net._nodes else "":
                        active_dnl_sats.add(a)
                    if "satellite" in net._nodes.get(b, type(None).__new__(type)).node_type if b in net._nodes else "":
                        active_dnl_sats.add(b)
            ...
            # Classify with combined states
            for sid in sat_ids:
                if sid == src_id or sid in route_sats:
                    states['transmitting'].append(sid)
                elif sid in tracking_sats and sid in active_isl_sats:
                    states['tracking_isl'].append(sid)
                elif sid in tracking_sats and sid in active_dnl_sats:
                    states['tracking_dnl'].append(sid)
        ...
        def create_earth_surface() -> go.Surface:
            """Create Earth surface that persists across frames"""
            R = 6378137.0
        ...
        def _frame_traces_enhanced(t: float, src_id: str, route: Optional[List[str]],
                                   target_trail: List, earth_surface: go.Surface) -> List:
            """Create enhanced frame traces with satellite state differentiation"""
            traces = [earth_surface]  # Always include Earth first

            # Classify satellites
            sat_states = classify_satellites_enhanced(t, route, src_id)
            ...
            if sat_states['idle']:
                ...
                traces.append(go.Scatter3d(
                    ...
                    showlegend=True,
                    legendgroup='sats',
                    hovertemplate='<b>Idle</b><br>%{text}<extra></extra>',
            ...
            # 3. TRACKING + ISL satellites
            if sat_states['tracking_isl']:
                track_isl_pos = np.array([sat_positions[sid] for sid in sat_states['tracking_isl']])
                traces.append(go.Scatter3d(
                    x=track_isl_pos[:, 0], y=track_isl_pos[:, 1], z=track_isl_pos[:, 2],
                    mode='markers',
                    marker=COLOR_SAT_TRACKING_ISL,
                    name='Satellites (Tracking+ISL)',
                    showlegend=True,
                    legendgroup='sats',
                    hovertemplate='<b>Tracking+ISL</b><br>%{text}<extra></extra>',
                    text=sat_states['tracking_isl']
                ))

            # 4. TRACKING + DOWNLINK satellites
            if sat_states['tracking_dnl']:
                track_dnl_pos = np.array([sat_positions[sid] for sid in sat_states['tracking_dnl']])
                traces.append(go.Scatter3d(
                    x=track_dnl_pos[:, 0], y=track_dnl_pos[:, 1], z=track_dnl_pos[:, 2],
                    mode='markers',
                    marker=COLOR_SAT_TRACKING_DNL,
                    name='Satellites (Tracking+Downlink)',
                    showlegend=True,
                    legendgroup='sats',
                    hovertemplate='<b>Tracking+DNL</b><br>%{text}<extra></extra>',
                    text=sat_states['tracking_dnl']
                ))

            # 5. TRANSMITTING satellites
            ...
            # 9. Active edges (ISL and Downlinks)
            net.update_topology(t)

            # Separate active and inactive ISLs/DNLs
            ...
            # 10. LOS rays to target
            los_xs, los_ys, los_zs = [], [], []
            los_count = 0
            tracking_all = (sat_states['tracking'] + sat_states['tracking_isl'] +
                            sat_states['tracking_dnl'])
            for sid in tracking_all:
        ...
        plotly_frames = []
        age_data = {'time': [], 'onboard': [], 'ground': []}
        earth_surface = create_earth_surface()

        print("🔧 Building frames with measurement age...")
        ...
        for idx, (t, src_id, _route) in enumerate(_tqdm(samples) if _tqdm else samples):
            ...
            # Get measurement ages
            onboard_age, ground_age = get_measurement_age_at_time(t, onboard_df, ground_df)
            age_data['time'].append(t)
            age_data['onboard'].append(onboard_age if onboard_age is not None else np.nan)
            age_data['ground'].append(ground_age if ground_age is not None else np.nan)

            traces = _frame_traces_enhanced(t, src_id, best_path, target_trail, earth_surface)
            ...
            sat_states = classify_satellites_enhanced(t, best_path, src_id)
            n_tracking = len(sat_states['tracking'])
            n_tracking_isl = len(sat_states['tracking_isl'])
            n_tracking_dnl = len(sat_states['tracking_dnl'])
            n_transmitting = len(sat_states['transmitting'])
            n_isl_active = len(sat_states['isl_active'])

            # Add measurement age info
            meas_age_text = ""
            if onboard_age is not None:
                meas_age_text += f"  • Onboard: {onboard_age:.1f} ms<br>"
            if ground_age is not None:
                meas_age_text += f"  • Ground: {ground_age:.1f} ms<br>"
            ...
            kpi_text = (
                ...
                f"<br>"
                f"<b>MEASUREMENT AGE:</b><br>"
                f"{meas_age_text}"
                f"<br>"
                f"<b>CONSTELLATION STATUS:</b><br>"
                f"  • Tracking: {n_tracking}<br>"
                f"  • Tracking+ISL: {n_tracking_isl}<br>"
                f"  • Tracking+DNL: {n_tracking_dnl}<br>"
            ...
            annotation = dict(
                ...
                borderwidth=1,
                font=dict(family="Arial, sans-serif", size=11, color="rgb(255, 255, 255)"),
                align="left",
                xanchor="left",
                yanchor="top"
            )
        ...
        # Build final figure with subplots
        if not plotly_frames:
            ...
            fig_pl = make_subplots(
                ...
                subplot_titles=('Satellite Constellation', 'Measurement Age (ms)')
            )

            # Add initial 3D traces
            for trace in plotly_frames[0].data:
                fig_pl.add_trace(trace, row=1, col=1)

            # Add measurement age plot
            if age_data['time']:
                # Onboard processing
                if not all(np.isnan(age_data['onboard'])):
                    fig_pl.add_trace(
                        go.Scatter(
                            x=age_data['time'],
                            y=age_data['onboard'],
                ...
                # Ground processing
                if not all(np.isnan(age_data['ground'])):
                    fig_pl.add_trace(
                        go.Scatter(
                            x=age_data['time'],
                            y=age_data['ground'],
                            ...
                            line=dict(color='rgba(255, 120, 0, 1)', width=2),
                            showlegend=True
                        ),
                        row=1, col=2
                    )

                # Add vertical line for current time (will be updated in frames)
                fig_pl.add_trace(
                    go.Scatter(
                        x=[age_data['time'][0], age_data['time'][0]],
                        y=[0,
                           max([max(age_data['onboard']), max(age_data['ground'])]) if age_data['onboard'] and age_data[
                               'ground'] else 100],
                        mode='lines',
                        line=dict(color='rgba(255, 255, 0, 0.7)', width=1, dash='dash'),
                        showlegend=False,
                        name='Current Time'
                    ),
                    row=1, col=2
            ...
            # Update frames to include age plot updates
            updated_frames = []
            for i, frame in enumerate(plotly_frames):
                # Update current time indicator
                if age_data['time']:
                    time_indicator = go.Scatter(
                        x=[age_data['time'][i], age_data['time'][i]],
                        y=[0,
                           max([max(age_data['onboard']), max(age_data['ground'])]) if age_data['onboard'] and age_data[
                               'ground'] else 100],
                        mode='lines',
                    ...
                    # Combine traces
                    all_traces = list(frame.data) + [
                        go.Scatter(x=age_data['time'][:i + 1], y=age_data['onboard'][:i + 1],
                                   mode='lines', line=dict(color='rgba(0, 255, 255, 1)', width=2),
                                   showlegend=False),
                        go.Scatter(x=age_data['time'][:i + 1], y=age_data['ground'][:i + 1],
                                   mode='lines', line=dict(color='rgba(255, 120, 0, 1)', width=2),
                                   showlegend=False),
                        time_indicator
                    ]
                else:
                    all_traces = frame.data

                updated_frames.append(go.Frame(
                    data=all_traces,
                    name=frame.name,
                    layout=frame.layout
                ))

            fig_pl.frames = updated_frames
            ...
            fig_pl.update_layout(
                ...
                xaxis2=dict(
                    title="Time (s)",
                    gridcolor="rgb(60, 65, 75)",
                    showgrid=True
                ),
                yaxis2=dict(
                    title="Measurement Age (ms)",
                    gridcolor="rgb(60, 65, 75)",
                    showgrid=True
                ),
                title={
                ...
                legend=dict(
                    ...
                    borderwidth=1,
                    font=dict(size=10, family='Arial, sans-serif', color='rgb(220, 220, 220)'),
                    yanchor="bottom",
                    y=0.01,
                    xanchor="right",
                    x=0.99
            ...
            if onboard_df is not None or ground_df is not None:
                print(f"📈 Measurement age data included!")
